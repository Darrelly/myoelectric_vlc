%% Collecting Data from 3 sEMG Sensors through XBee
% Description: Collecting Raw EMG data from 3 sEMG sensors
...
for gest = 1:size(gestures,2)
    ...
    % append data to multiclass data vector, label data
...
training_table = array2table(training_data,...
    'VariableNames',{'sEMG1','sEMG2','sEMG3','x','y','z'});

training_table_labeled = addvars(training_table,training_labels);

% Save file
train_filename = nextname('sEMG_3_XBee_accel_train_','01','.csv');
...
% Save file
test_filename = nextname('sEMG_3_XBee_accel_test_','01','.csv');
...
while s + window < data_points
    ...
    if all(ismember(T1.training_labels(segment), curr_label))
        ...
        T1_windowed(ind,13)    = mean([T1.x(segment)]);
        T1_windowed(ind,14)    = mean([T1.y(segment)]);
        T1_windowed(ind,15)    = mean([T1.z(segment)]);
...
while s + window < data_points
    ...
    if all(ismember(T2.testing_labels(segment), curr_label))
        ...
        T2_windowed(ind,13)    = mean([T2.x(segment)]);
        T2_windowed(ind,14)    = mean([T2.y(segment)]);
        T2_windowed(ind,15)    = mean([T2.z(segment)]);
